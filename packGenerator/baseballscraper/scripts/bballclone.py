...
def get_pack(size):
    ...
    while(y<size):
        ...
        x = 0
        if (pagetry == totalpages):
            tries = totalcards%50
        else:
            tries = 50

        firsttry = randint(1, tries)

        while (x < tries):
            
            tryonpage = (firsttry + x)%50
            cardindex = tryonpage + 2

            try:
                if cards[cardindex].a.img["src"] == "/Images/AddCard2.gif":
                    x+=1
                    continue
                imglink = "https://www.tcdb.com" + cards[cardindex].a["href"]
                uClient = uReq(imglink)
                n_page_html = uClient.read()
                uClient.close()
            ...
            snippet = soup(n_page_html, "lxml", parse_only=product)
